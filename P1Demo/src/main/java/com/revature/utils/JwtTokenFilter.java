...
//This will get called for every request that hits our server (even if it doesn't need a JWT)
...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    private String getAccessToken(HttpServletRequest request) {
        String header = request.getHeader("Authorization");
        String token = header.split(" ")[1].trim(); //erase the "Bearer" part, saving just the JWT
        return token;
    }
